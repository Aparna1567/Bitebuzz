...
const Banner = () => {
  ...
  // };
{/* Add this CSS in your index.css or Banner component */}
<style>
  {`
    @keyframes float-up {
...
<div className="relative w-full h-16 mt-6 flex justify-center gap-4">
  {['🍕', '🍔', '🍣', '🌮','🥤', '🍜'].map((emoji, idx) => {
    const delay = Math.random() * 0.5; // random start delay
    const duration = 1 + Math.random(); // random bounce speed
    return (
      <div
        key={idx}
        style={{
          ...
          fontSize: '2rem',
          animation: `bounce ${duration}s ${delay}s infinite`,
        }}
      >
        {emoji}
      </div>
