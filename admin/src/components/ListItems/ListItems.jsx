...
const ListItems = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const { data } = await axios.get('https://bitebuzz-backend-ue5i.onrender.com/api/items');
        setItems(data);
      } catch (err) {
  ...
  const handleDelete = async (itemId) => {
    ...
    try {
      await axios.delete(`https://bitebuzz-backend-ue5i.onrender.com/api/items/${itemId}`);
